...
class Inspire_Controller_DFX:
    def __init__(self, left_hand_array, right_hand_array, dual_hand_data_lock = None, dual_hand_state_array = None,
        ...
        wait_count = 0
        while True:
            left_ready = (not enable_left) or any(self.left_hand_state_array)
            right_ready = (not enable_right) or any(self.right_hand_state_array)
            ...
            if wait_count % 100 == 0:
                logger_mp.warning(
                    f"[Inspire_Controller_DFX] Waiting to subscribe dds... "
                    f"left_ready={left_ready}, right_ready={right_ready}"
                )
            if wait_count > 500:
                logger_mp.warning("[Inspire_Controller_DFX] Timeout waiting for enabled hand states. Proceeding anyway.")
                break
    ...
    def ctrl_dual_hand(self, left_q_target, right_q_target, enable_left=True, enable_right=True):
        """
    ...
    def control_process(self, left_hand_array, right_hand_array, left_hand_state_array, right_hand_state_array,
        ...
        try:
            while self.running:
                ...
                left_state_snapshot = np.array(left_hand_state_array[:])
                right_state_snapshot = np.array(right_hand_state_array[:])
                state_data = np.concatenate((left_state_snapshot, right_state_snapshot))
                if not enable_left and any(left_state_snapshot):
                    left_q_target = left_state_snapshot.copy()
                if not enable_right and any(right_state_snapshot):
                    right_q_target = right_state_snapshot.copy()
                ...
                self.ctrl_dual_hand(left_q_target, right_q_target, enable_left=enable_left, enable_right=enable_right)
...
class Inspire_Controller_FTP:
    def __init__(self, left_hand_array, right_hand_array, dual_hand_data_lock = None, dual_hand_state_array = None,
        ...
        from teleop.robot_control.devices.inspire_ftp import messages as inspire_dds  # lazy import
        from teleop.robot_control.devices.inspire_ftp import defaults as inspire_hand_default
        self.fps = fps
        self.Unit_Test = Unit_Test
        self.simulation_mode = simulation_mode
        self.enable_left = enable_left
    ...
    def _send_hand_command(self, left_angle_cmd_scaled, right_angle_cmd_scaled, enable_left=True, enable_right=True):
        """
        ...
        if self._debug_count < 50:
            logger_mp.info(f"[Inspire_Controller_FTP] Publish cmd L={left_angle_cmd_scaled if enable_left else 'disabled'} R={right_angle_cmd_scaled if enable_right else 'disabled'} ")
            self._debug_count += 1
    ...
    def control_process(self, left_hand_array, right_hand_array, left_hand_state_array, right_hand_state_array,
        ...
        try:
            while self.running:
                ...
                self._send_hand_command(scaled_left_cmd, scaled_right_cmd, enable_left=enable_left, enable_right=enable_right)
                current_time = time.time()
                time_elapsed = current_time - start_time
                sleep_time = max(0, (1 / self.fps) - time_elapsed)
                time.sleep(sleep_time)
