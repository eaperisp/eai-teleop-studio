"""Standalone Inspire DFX dexterous hand test tool."""
...
from teleop.robot_control.devices.inspire_dfx.sdk import (  # noqa: E402
    CLOSE_ANGLES,
    HAND_SIDES,
    OPEN_ANGLES,
    PINCH_ANGLES,
...
def print_state(sdk, sides, timeout):
    for side in sides:
        ...
        if state is None:
            ...
            print(f"{side}: q={list(state.angles)}")
...
def main():
    parser = argparse.ArgumentParser(description="Test Inspire DFX hand DDS topics.")
    ...
    parser.add_argument("--values", nargs="*", help="One angle or six angles for command=angle, range 0.0-1.0.")
    ...
    sdk.initialize()
    print(
        f"Inspire DFX hand test started: side={args.side}, command={args.command}, "
