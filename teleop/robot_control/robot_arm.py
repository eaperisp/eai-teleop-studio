...
class H2_ArmController:
    def __init__(self, motion_mode=False, simulation_mode=False):
        ...
        self._arm_indices = set(member.value for member in H2_JointArmIndex)
        self._locked_joint_indices = [
            member
            for member in H2_JointIndex
            if member.value not in self._arm_indices and not member.name.startswith("kNotUsed")
        ]
        self._locked_joint_values = {member.value for member in self._locked_joint_indices}
        self.locked_q_target = self.all_motor_q.copy()
        logger_mp.debug(f"Current all body motor state q:\n{self.all_motor_q} \n")
        logger_mp.debug(f"Current two arms motor state q:\n{self.get_current_dual_arm_q()}\n")
        logger_mp.info("Lock all joints except two arms...")

        if self.motion_mode:
            logger_mp.info("H2 motion mode: only arm joints and arm_sdk weight are published to rt/arm_sdk.")
            for id in H2_JointArmIndex:
                # Match Unitree's H2 arm_sdk DDS example: do not set motor mode
                # on rt/arm_sdk; publish q/dq/kp/kd/tau plus the weight field.
                self.msg.motor_cmd[id].kp = 80.0
            ...
            logger_mp.info("H2 arm_sdk arm-only command initialized.")
        else:
            for id in H2_JointIndex:
                self.msg.motor_cmd[id].mode = 1
                if id.value in self._arm_indices:
                    if self._Is_wrist_motor(id):
                        self.msg.motor_cmd[id].kp = self.kp_wrist
                        self.msg.motor_cmd[id].kd = self.kd_wrist
                    else:
    ...
    def _ctrl_motor_state(self):
        ...
        while True:
            ...
            with self.ctrl_lock:
                ...
                locked_q_target = self.locked_q_target.copy()
            ...
            if self.motion_mode:
                ...
                for id in self._locked_joint_indices:
                    self.msg.motor_cmd[id].q = locked_q_target[id]
                    self.msg.motor_cmd[id].dq = 0
    ...
    def set_locked_joint_targets(self, joint_targets):
        """Set H2 non-arm joint targets that should remain locked during arm teleoperation."""
        if self.motion_mode:
            return
        with self.ctrl_lock:
            for joint_index, q_target in joint_targets.items():
                joint_index = joint_index if isinstance(joint_index, H2_JointIndex) else H2_JointIndex(joint_index)
                if joint_index.value not in self._locked_joint_values:
                    continue
                self.locked_q_target[joint_index] = float(q_target)
                self.msg.motor_cmd[joint_index].q = float(q_target)
