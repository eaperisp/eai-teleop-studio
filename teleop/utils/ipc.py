...
class IPC_Server:
    ...
    def __init__(self, on_press=None, get_state=None, on_arm_target=None, hb_fps=10.0):
        ...
        self.on_arm_target = on_arm_target if callable(on_arm_target) else None
    ...
    def _handle_message(self, msg: dict) -> dict:
        ...
        try:
            ...
            # unsupported cmd
            if cmd == "CMD_SET_ARM_TARGET":
                if self.on_arm_target is None:
                    return {"repid": reqid, "status": "error", "msg": "arm target callback not configured"}
                self.on_arm_target(msg)
