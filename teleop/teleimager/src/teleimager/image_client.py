...
class ZMQ_SubscriberThread(threading.Thread):
    ...
    def __init__(
        ...
        self._started = threading.Event()
        try:
            self._stale_timeout = max(0.2, float(os.environ.get("XR_TELEOP_ZMQ_STALE_TIMEOUT", "2.0")))
        except ValueError:
                        ...
                        if not self._has_marked_stale:
                            ...
                            if self._request_bgr:
                                try:
                                    ...
                                    pass
                            self._has_marked_stale = True
                        self._fps_monitor.reset()
                        if now - self._last_stale_log_at >= 5.0:
                            logger_mp.warning(
                                f"No message received from {self._host}:{self._port} "
                                f"for {self._stale_timeout:.1f}s; marking stream stale."
                            )
                            self._last_stale_log_at = now
                    else:
                        logger_mp.debug(
                            f"No new message from {self._host}:{self._port}; keeping latest frame."
                        )
        except Exception as e:
...
class ImageClient:
    ...
    def get_enabled_zmq_camera_names(self):
        return [
            camera_name
            for camera_name, camera_cfg in self._cam_config.items()
    ...
    def get_left_wrist_frame(self):
        return self.get_camera_frame('left_wrist_camera')
    
    def get_right_wrist_frame(self):
        return self.get_camera_frame('right_wrist_camera')
